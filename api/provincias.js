...
export default async function handler(req, res) {
  try {
    ...
    return ok(res, data, { collection: "provincias", total: data.length });
  } catch (error) {
    console.error("Error en /api/provincias:", error);
    return fail(res, 500, "No se pudo obtener provincias");
  }
}